...
class MyGeoTFNO(nn.Module):
    def __init__(self, n_modes, in_channels, hidden_channels,eigs):
        ...
        self.tfno=TFNO(n_modes=(64,64),in_channels=in_channels ,hidden_channels=32)
        self.lin0=nn.Linear(m,64*64)
        self.lin1=nn.Linear(64*64,m)

    def forward(self, x):
        x=x.reshape(-1,m)
        x = self.lin0(x).reshape(-1,1,64,64)
        x = self.tfno(x)
        x= self.lin1(x.reshape(-1,64*64)) 
        return x
model=MyGeoTFNO(n_modes=64,in_channels=1 ,hidden_channels=32,eigs=eigs)
'''
